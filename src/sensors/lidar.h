...
struct Lidar
{
	...
	std::vector<Car> cars;
	Vect3 position;     // 실제로 라이더가 위치할 곳 / 시각화 할때 rendeRays 함수의 매개변수 중 라이다의 위치 (const Vect3& origin) 부분에 lidar->position 쓰는 이유
	double groundSlope;
	double minDistance;
	double maxDistance;
	double resoultion;
	...
	Lidar(std::vector<Car> setCars, double setGroundSlope) // 생성자 // 매개변수 2개 - setCars(차), setGroundSlope(지상경사값) 레이캐스팅에 사용됨
		: cloud(new pcl::PointCloud<pcl::PointXYZ>()), position(0,0,2.6)
	{
		// TODO:: set minDistance to 5 to remove points from roof of ego car
		minDistance = 5; //라이더와 너무 가까우면 그 지점은 차의 지붕(라이더 주변)으로 가정 //  원래 0;
		maxDistance = 50;
		resoultion = 0.2;
		...
		double horizontalAngleInc = pi/64; // 수평레이어 증가 해상도 (?)//원래  pi/6;
